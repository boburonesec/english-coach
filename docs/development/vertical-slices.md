...
4. **Realtime voice conversation — implemented; live qualitative acceptance requires credentials.** The server-owned prompt creates a natural B1-level topic discussion with communication before correction. The browser activates the microphone only after verified session readiness, plays remote speech, supports explicit Hint and graceful End Conversation, and cleans resources for retry.
